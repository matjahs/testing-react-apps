...
Note, you're going to need to use `act` when calling `result.increment` etc.
...
Also, keep in mind that `@testing-library/react-hooks` exports its own `act` and
you'll want to use that one instead of the one exported by
...
And use the `rerender` function you get from `renderHook` to test an update to
the step prop.
...
https://ws.kcd.im/?ws=Testing%20React%20Applications%20%F0%9F%A7%90&e=08%3A%20testing%20custom%20hooks&em=matjah%40matjah.eu
