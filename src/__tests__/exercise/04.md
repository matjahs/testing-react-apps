...
🦺 The `overrides` object that you passed should be optional and it's type can
be set to `Partial<LoginFormValues>`. You can get `LoginFormValues` via
...
It provides a few nice utilities (including the ability to provide overrides
like our own function does). Check out the docs there and swap your custom
`buildLoginForm` with one you create using the Test Data Bot.

🦺 the `build` function in `test-data-bot` is a generic. You can pass the
`LoginFormValues` to that to make sure you're generating all the right fields.
...
https://ws.kcd.im/?ws=Testing%20React%20Applications%20%F0%9F%A7%90&e=04%3A%20form%20testing&em=matjah%40matjah.eu
